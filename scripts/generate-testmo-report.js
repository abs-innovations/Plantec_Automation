...
function appendTestmoRunLinkingArgs(args, env = process.env) {
  ...
  const milestoneName = String(env.TESTMO_MILESTONE_NAME || env.TESTMO_MILESTONE || '').trim();
  const configuredTags = parseTestmoTags(env.TESTMO_MILESTONE_TAG || env.TESTMO_TAGS);
  let appliedTags = [];
 
  // Priority: milestone ID -> milestone name -> automation tags.
  // This prevents one automation run from being linked through conflicting methods.
  if (milestoneId) {
...
function publishTestmoResults(env = process.env) {
  ...
  const runLinking = appendTestmoRunLinkingArgs(createArgs, env);
 
  if (runLinking.milestoneId) {
    console.log(`Testmo milestone ID: ${runLinking.milestoneId}`);
  } else if (runLinking.milestoneName) {
    console.log(`Testmo milestone name: ${runLinking.milestoneName}`);
  } else {
    console.log('Testmo milestone: not configured');
  }
 
  console.log(`Testmo tags: ${runLinking.tags.length ? runLinking.tags.join(', ') : '-'}`);
 
  createArgs.push('--no-ansi');
 
  const createOutput = runCommand(
    testmoCmd,
    withNpxPackage(testmoCmd, createArgs),
    childEnv,
  );
