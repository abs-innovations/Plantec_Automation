...
├── testData.ts              # Test data, constants, scenarios
├── helpers.ts               # Common helper functions
├── test-data/               # Profile JSON files
├── scripts/                 # Reporting and integration scripts
├── playwright.config.ts
├── playwright.ffb.config.ts

All tests using `harvestingPage.save()` automatically use the new selector.

## 🧪 Using Helpers

`package.json` and `package-lock.json` remain at the repository root so npm dependencies and scripts are shared globally.
