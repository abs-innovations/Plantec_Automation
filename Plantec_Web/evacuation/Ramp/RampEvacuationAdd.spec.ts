...
async function selectFirstValidOption(selectLocator: ReturnType<Page['locator']>) {
  ...
  throw new Error('No valid option available in dropdown.');
}

async function selectRamp(page: Page) {
  const rampSelect = page.locator('#rampId').first();
  if (await rampSelect.isVisible().catch(() => false)) {
    await selectFirstValidOption(rampSelect);
    return;
  }

  const fallbackRampSelect = page.locator('label:has-text("Ramp")').locator('xpath=following::select[1]').first();
  if (await fallbackRampSelect.isVisible().catch(() => false)) {
    await selectFirstValidOption(fallbackRampSelect);
    return;
  }

  throw new Error('Ramp dropdown was not found on the form.');
}
...
test('[Ramp Evacuation] Add record with FFB date filter selection', async ({ page }) => {
  ...
  await test.step('Select estate and Ramp evacuation type', async () => {
    await evacuationPage.selectEstateByValue(EVACUATION_DATA.estateValue);
    await selectEvacuationType(page, /ramp\s*evacuation/i);
  });

  await test.step('Select Ramp, Vehicle, Driver, Loader and one FFB record', async () => {
    await selectRamp(page);
